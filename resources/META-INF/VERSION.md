...
- [2.1.1.48 - Bug Fix & Enhancement Release](#21148---bug-fix--enhancement-release)
...
* [ ] splitting a line right after bullet item marker (not task item) does not create a an item
      with the split off text but plain text line, which makes all following items lazy
      continuation of this paragraph. Any typing converts the list items to jumbled paragraph
      text. Need to allow list items to interrupt paragraphs for maximum safety and also
      compatibility on other parsers. Just make an annotation with a warning for it.
* [ ] wrapping a `*`, `-`, `+`, `#` _`N.`_ where _N_ is numeric should be optionally escaped
      with a `\` so that it is not interpreted as a special char. If one is found in such a
      position then it should be annotated with a warning.
...
* [ ] Fix: Link Map
      ...
      * [ ] add quick fix to move errant mapping group to `unused` link type so config can be
            saved.
...
### 2.1.1.48 - Bug Fix & Enhancement Release
