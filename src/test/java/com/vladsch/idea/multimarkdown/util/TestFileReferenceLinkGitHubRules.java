...
public class TestFileReferenceLinkGitHubRules {
    ...
    public TestFileReferenceLinkGitHubRules(
        ...
        fileReferenceLink = new FileReferenceLinkGitHubRules(sourceReference, targetReference, null);
    ...
    private static Object[] filePathInfoTestData(
        ...
        String refLink = Paths.get("/" + FilenameUtils.getPath(sourceReferenceNoAnchor)).relativize(Paths.get(targetReferenceNoAnchor)).toString();
        String refLinkWithAnchor = Paths.get("/" + FilenameUtils.getPath(sourceReference)).relativize(Paths.get(targetReference)).toString();
        FilePathInfo info;
        String adjustedTargetReferenceNoAnchor = (info = new FilePathInfo(sourceReferenceNoAnchor).resolveLinkRef(refLink, true, false)) != null ? info.getFilePath() : null;
        String adjustedTargetReference = (info = new FilePathInfo(sourceReference).resolveLinkRef(refLinkWithAnchor, true, true)) != null ? info.getFilePathWithAnchor() : null;

        result[0] = sourceReference;
        result[1] = targetReference;
        result[2] = Paths.get("/" + FilenameUtils.getPath(sourceReferenceNoAnchor)).relativize(Paths.get(adjustedTargetReferenceNoAnchor)).toString();
        result[3] = FilenameUtils.getBaseName((String) result[2]).replace('-', ' ');
        ...
        result[8] = Paths.get("/" + FilenameUtils.getPath(sourceReference)).relativize(Paths.get(adjustedTargetReference)).toString();
        result[9] = FilenameUtils.getBaseName((String) result[8]).replace('-', ' ');
        return result;
    }
}
