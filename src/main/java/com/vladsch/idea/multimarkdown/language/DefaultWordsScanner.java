    ...
    public void processWords(CharSequence fileText, Processor<WordOccurrence> processor) {
        ...
        while ((type = myLexer.getTokenType()) != null) {
            if (myIdentifierTokenSet.contains(type)) {
                ...
                // we can have spaces and all kinds of crap in the wiki link
                occurrence.init(fileText, myLexer.getTokenStart(), myLexer.getTokenEnd(), WordOccurrence.Kind.CODE);
                if (!processor.process(occurrence)) return;

                occurrence.init(fileText, myLexer.getTokenStart(), myLexer.getTokenEnd(), WordOccurrence.Kind.FOREIGN_LANGUAGE);
                if (!processor.process(occurrence)) return;
                ...
                occurrence.init(fileText, myLexer.getTokenStart(), myLexer.getTokenEnd(), WordOccurrence.Kind.LITERALS);
                if (!processor.process(occurrence)) return;
