...
public class JavaFxCouldBeEnabledNotificationProvider extends EditorNotifications.Provider<EditorNotificationPanel> implements DumbAware {
    ...
    public EditorNotificationPanel createNotificationPanel(@NotNull VirtualFile file, @NotNull final FileEditor fileEditor, @NotNull Project project) {
        ...
        if (MdApplicationSettings.getInstance().getWasShownSettings().getJavaFxAvailable()) {
            return null;
        }
        ...
        final HtmlPanelProvider.AvailabilityInfo availabilityInfo = new JavaFxHtmlPanelProvider().isAvailable();
        ...
        panel.setText(MdBundle.message("editor.javafx.is.available"));

        panel.createActionLabel(MdBundle.message("editor.javafx.change.to"), () -> {
            ...
            if (isSuccess) {
                ...
                MdRenderingProfile newSettings = renderingSettings.changeToProvider(renderingSettings.getPreviewSettings().getHtmlPanelProviderInfo(), JavaFxHtmlPanelProvider.INFO);
                ...
                Logger.getInstance(JavaFxCouldBeEnabledNotificationProvider.class).warn("Could not apply OpenJFX");
            }
        });

        panel.createActionLabel(MdBundle.message("editor.dont.show.again"), () -> {
            MdApplicationSettings.getInstance().getWasShownSettings().setJavaFxAvailable(true);
            EditorNotifications.updateAll();
        });
        return panel;
    }
