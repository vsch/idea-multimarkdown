...
public abstract class SplitTextEditorProvider implements AsyncFileEditorProvider, DumbAware {
    ...
    private static final String SPLIT_PREVIEW = "split_preview";
    ...
    public FileEditorState readState(@NotNull Element sourceElement, @NotNull Project project, @NotNull VirtualFile file) {
        ...
        final Attribute layoutAttribute = sourceElement.getAttribute(SPLIT_LAYOUT);

        final String layoutName;
        if (layoutAttribute != null) {
            layoutName = layoutAttribute.getValue();
        } else {
            layoutName = null;
        }

        final Attribute attribute = sourceElement.getAttribute(SPLIT_PREVIEW);

        final String previewName;
        if (attribute != null) {
            previewName = attribute.getValue();
        } else {
            previewName = null;
        }

        return new SplitFileEditor.MyFileEditorState(layoutName, previewName, firstState, secondState);
    }
    ...
    @NotNull
    private static Builder getBuilderFromEditorProvider(
            @NotNull final FileEditorProvider provider,
