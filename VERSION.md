...
- [3.0.213.150 - Bug Fix Release](#30213150---bug-fix-release)
- [3.0.xxx.149 - Bug Fix Build](#30xxx149---bug-fix-build)
- [3.0.xxx.145 - Bug Fix Build](#30xxx145---bug-fix-build)
- [3.0.xxx.143 - Bug Fix Build](#30xxx143---bug-fix-build)
- [3.0.xxx.141 - Bug Fix Build](#30xxx141---bug-fix-build)
- [3.0.xxx.139 - Bug Fix Build](#30xxx139---bug-fix-build)
- [3.0.xxx.137 - Bug Fix Build](#30xxx137---bug-fix-build)
- [3.0.xxx.135 - Bug Fix Build](#30xxx135---bug-fix-build)
- [3.0.xxx.133 - Bug Fix Build](#30xxx133---bug-fix-build)
- [3.0.xxx.131 - Bug Fix Build](#30xxx131---bug-fix-build)
- [3.0.xxx.129 - Bug Fix Build](#30xxx129---bug-fix-build)
- [3.0.xxx.127 - Feature Build](#30xxx127---feature-build)
- [3.0.xxx.125 - Feature Build](#30xxx125---feature-build)
- [3.0.xxx.123 - Feature Build](#30xxx123---feature-build)
- [3.0.xxx.121 - Feature Build](#30xxx121---feature-build)
- [3.0.xxx.119 - Bug Fix Build](#30xxx119---bug-fix-build)
- [3.0.xxx.117 - Bug Fix Build](#30xxx117---bug-fix-build)
- [3.0.xxx.115 - Bug Fix Release](#30xxx115---bug-fix-release)
- [3.0.xxx.114 - Bug Fix Release](#30xxx114---bug-fix-release)
...
### 3.0.213.150 - Bug Fix Release

* Change: default list item continuation text alignment to list item text, instead of first item
  left text edge.
* Fix: list item wrap on typing ignores left margin : `List item text` alignment. Always wraps
  as if `Align left text edge`
* Fix: `BundleBase` use instead of `AbstractBundle`.
* Fix: use of internal api for determining module dir
* Fix: change bundle classes to extend `AbstractBundle` for `232` compatibility.
* Fix: replace `javafx` providers with `jbcef` providers on settings loading.
* Fix: broken plugin path code due to plugin distribution file name change.
* Fix: missing `emojis` directory in distribution zip, broken by gradle build migration
* Fix: flexmark spec example tests.
* Fix: flexmark html settings not being saved.
* Fix: tab/backtab overrides to not bother with exact state (enabled/disabled) and apply exact
  state in `beforeActionUpdate()`. Seems it takes a long time to figure out the state for some
  reason, not yet investigated.
* Remove: diagnostics settings extension
* Remove: firehose all unused/unneeded code
  * legacy license code
  * diagnostics/error reporting code
  * unused flexmark module creation classes
* Fix: deprecations where possible
* Fix: exclude `slf4j` from transient dependencies.
* Fix: avoid potential recursion on reparse markdown files when invoked in dumb mode.
* Change: update flexmark:0.64.4, and other dependencies
* Change: remove JavaFX WebView based preview support.
* Add: `HtmlFontWeight` enum
* Fix: exporter causing `Read access is allowed from inside read-action (or EDT)`
* Add: Convert to Bitmap dialog to convert ASCII to monochrome bitmaps. Used in arduino console
  debugging of graphical display but may be useful in other cases where ASCII is used to output
  graphics.
* Fix: light files should have PSI only in one project
* Fix: broken mouse highlights code image dialog
* Add: image size status to paste/copy/modify image dialog
* Add: rotation option to paste/copy/modify image dialog.
* Fix: `getFile()` warning on startup
* Fix: Opening a project and closing it can cause AlreadyDisposedException project is already
  disposed.
* Fix: 2021.3 compatibility issues

### 3.0.xxx.149 - Bug Fix Build

* Fix: NEW-FEATURES.md to remove outdated info and links.
* Fix: rename string properties in enh module
* Change: default list item continuation text alignment to list item text, instead of first item
  left text edge.

### 3.0.xxx.145 - Bug Fix Build
...
### 3.0.xxx.141 - Bug Fix Build

* Fix: flexmark spec example tests.
* Fix: flexmark html settings not being saved.

### 3.0.xxx.139 - Bug Fix Build

* Fix: tab/backtab overrides to not bother with exact state (enabled/disabled) and apply exact
  state in `beforeActionUpdate()`. Seems it takes a long time to figure out the state for some
  reason, not yet investigated.

### 3.0.xxx.137 - Bug Fix Build

* Remove: diagnostics settings extension

### 3.0.xxx.135 - Bug Fix Build
...
### 3.0.xxx.133 - Bug Fix Build

* Fix: exclude `slf4j` from transient dependencies.

### 3.0.xxx.131 - Bug Fix Build
...
| Value |    Common weight name     |           Enum Values            |
|-------|---------------------------|----------------------------------|
| 100   | Thin (Hairline)           | FW_100, THIN, HAIRLINE           |
| 200   | Extra Light (Ultra Light) | FW_200, EXTRA_LIGHT, ULTRA_LIGHT |
| 300   | Light                     | FW_300, LIGHT                    |
| 400   | Normal                    | FW_400, NORMAL                   |
| 500   | Medium                    | FW_500, MEDIUM                   |
| 600   | Semi Bold (Demi Bold)     | FW_600, SEMI_BOLD, DEMI_BOLD     |
| 700   | Bold                      | FW_700, BOLD                     |
| 800   | Extra Bold (Ultra Bold)   | FW_800, EXTRA_BOLD, ULTRA_BOLD   |
| 900   | Black (Heavy)             | FW_900, BLACK, HEAVY             |

### 3.0.xxx.129 - Bug Fix Build

* Fix: exporter causing `Read access is allowed from inside read-action (or EDT)`

### 3.0.xxx.127 - Feature Build
...
### 3.0.xxx.125 - Feature Build

* Fix: broken mouse highlights code image dialog

### 3.0.xxx.123 - Feature Build

* Add: image size status to paste/copy/modify image dialog

### 3.0.xxx.121 - Feature Build

* Add: rotation option to paste/copy/modify image dialog.

### 3.0.xxx.119 - Bug Fix Build

* Fix: `getFile()` warning on startup

### 3.0.xxx.117 - Bug Fix Build
...
* Fix: javafx tools debugger to handle failure when running Java 14 with alternate JavaFX
  Implementation.

  For example for Java 14 and installation of
  [Gluon JavaFX](https://gluonhq.com/products/javafx/) to
  `/Library/Java/Extensions/javafx-sdk-14.0.1` requires running the IDE with additional vm
  options:
...
* Fix: smart edit chars: `*`, `_`, `~` and `` ` ``, on space to delete auto-inserted char and
  allow space processing to proceed. Otherwise, auto-wrap on space only does not wrap after
  auto-inserted character is replaced by a space.

### 3.0.xxx.97 - Bug Fix Release

* Fix: diagnostic/4988, `IndexOutOfBoundsException` in `MoveColumnLeftAction`
* Fix: diagnostic/4985, `IndexOutOfBoundsException` in `MdFoldingVisitor.updateHeadingRanges()`
...
* Fix: multi-file drop could mess up links for files after the first dropped file.
...
* Fix: [#848, Markdown panel does not appear on scratch files - Pycharm 2020.1 Pro], reverted
  file type test code for editor provider to file language type test which works for scratch
  files created via `New File` action in `Scratches` directory.
* Fix: make all plugin topics to not broadcast to parent/child message buses.
* Fix: [#844, Jekyll include elements do not show included content in preview]
* Fix: [#828, Memory leak when project is closed due to MdProjectSettings]
* Fix: Tab/Back-Tab list indentation for items at end of file, without an EOL and for empty list
  items with no space after marker.
* Fix: paragraph wrapping context at end of line after editing.

### 3.0.xxx.89 - Bug Fix Release

* Fix: diagnostic/4558, concurrent modification in fenced code conversion manager.
* Fix: [#840, Edit Table of Contents style intention dialog has no effect], toc style edit
  ignores title level setting if title text has leading `#` chars.
* Fix: wrap on typing removing footnote and definition prefix, resulting in indented code
  instead of the original element.

### 3.0.xxx.87 - Bug Fix Release

* Add: Inspections for math, mermaid and plantuml to enable required settings if with code
  fenced elements for these are found in the file.
* Fix: diagnostic/4843, tracked position mismatch after wrapping.
* Fix: [#840, Edit Table of Contents style intention dialog has no effect], missing update of
  title level in form
